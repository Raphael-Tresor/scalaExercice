...
trait Solver extends GameDef:
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    /** We compute the position attained from X+1 step when started from a possition attained
     * in X step.
     * That is why in argument initials all histories have the same lenght.
     */
    def fromBreadthX(positions: List[(Block, List[Move])], explored: Set[Block]): LazyList[List[(Block, List[Move])]]=
      val newPositions =
        for {
        (block,moves) <- positions
        newPosition <- newNeighborsOnly(neighborsWithHistory(block,moves), explored)
      } yield newPosition
      positions #:: fromBreadthX(newPositions,explored ++ newPositions.map(_._1))
    for
      positions <- fromBreadthX(List(initial.head), explored)
      position <- positions
    yield position
  /**
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] =
    pathsFromStart.filter((block:Block, _) => done(block))
   ...
   * position.
   */
  lazy val solution: List[Move] = pathsToGoal.take(1).head._2.reverse
