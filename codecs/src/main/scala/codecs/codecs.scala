...
trait DecoderInstances:
  ...
  given Decoder[Int] =
    Decoder.fromPartialFunction { case num:Json.Num if num.value.isValidInt => num.value.toInt }

  /** A decoder for `String` values */
  // TODO Define a given value of type `Decoder[String]`
  given Decoder[String] =
    Decoder.fromPartialFunction { case string:Json.Str => string.value }

  /** A decoder for `Boolean` values */
  // TODO Define a given value of type `Decoder[Boolean]`
  given Decoder[Boolean] =
    Decoder.fromPartialFunction { case bool:Json.Bool => bool.value }
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
      case json:Json.Arr => json.items.foldRight(Some(List()):Option[List[A]]){
        (innerJson,list) => for {
        decodedList <- list
        decodedJson <- decoder.decode(innerJson)  } yield decodedJson :: decodedList
      }
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction { json => json match
      case obj:Json.Obj => decoder.decode(obj.fields(name))
      case _ => None
      }
...
trait ContactsCodecs:
  ...
  given Encoder[Contacts] =
    ObjectEncoder.field[List[Person]]("people").transform(contacts => contacts.people)
