...
object ParallelCountChange extends ParallelCountChangeInterface:
  ...
  def countChange(money: Int, coins: List[Int]): Int =
    if coins == Nil then 0 else
      money - coins.head match
        case 0 => 1
        case i if (i < 0) => countChange(money,coins.tail)
  ...
  def parCountChange(money: Int, coins: List[Int], threshold: Threshold): Int =
    if threshold(money,coins) then countChange(money,coins) else
      if coins == Nil then 0 else
        money - coins.head match
          case 0 => 1
          case i if (i < 0) => parCountChange(money, coins.tail, threshold)
          case _ =>
            val (countA,countB) = parallel(parCountChange(money - coins.head, coins,threshold) , parCountChange(money, coins.tail,threshold))
            countA + countB
  ...
  def combinedThreshold(startingMoney: Int, allCoins: List[Int]): Threshold =
    (money:Int, coins:List[Int]) => coins.length*money <= (startingMoney * allCoins.length)/2
