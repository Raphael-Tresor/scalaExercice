...
case class Fork(
  ...
  val massX: Float = if mass == 0 then centerX else (nw.massX*nw.mass   + ne.massX*ne.mass + se.massX*se.mass + sw.massX*sw.mass )/mass
  val massY: Float = if mass == 0 then centerY else (nw.massY*nw.mass   + ne.massY*ne.mass + se.massY*se.mass + sw.massY*sw.mass )/mass
...
extends Quad:
  val (mass, massX, massY) = (bodies.foldLeft(0)(_+_.mass) : Float, bodies.foldLeft(0)(_+_.x)/total : Float,bodies.foldLeft(0)(_+_.y)/total  : Float)
  val total: Int = bodies.length
  def insert(b: Body): Quad = if total >= minimumSize then bodies.foldLeft(Fork(Empty,Empty,Empty,Empty))((fork,b)=>fork.insert(b)) else Leaf(this.centerX,this.centerY,this.size+1,b + bodies)
...
class Body(val mass: Float, val x: Float, val y: Float, val xspeed: Float, val yspeed: Float):
  ...
  def updated(quad: Quad): Body =
    ...
    def traverse(quad: Quad): Unit = (quad: Quad) match
      ...
      case Fork(nw, ne, sw, se) => if distance(this.x,this.y,quad.centerX,quad.centerY)  > theta then
        traverse(nw); traverse(ne);traverse(sw);traverse(se) else addForce(quad.mass,quad.massX,quad.massY)
